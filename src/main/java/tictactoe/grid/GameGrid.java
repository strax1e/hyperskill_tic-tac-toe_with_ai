...
public class GameGrid {
    ...
    public boolean isBusy(GridCoordinate coordinate) {
        return getSymbolByCoordinate(coordinate) != GridSymbol.EMPTY;
    }
    ...
    public GameStatus getStatus() {
        for (int indexOfRow = 0; indexOfRow < SIZE; indexOfRow++) {
            if (isOnRow(indexOfRow)) {
                return GameStatus.getStatusBy(grid[indexOfRow][0]);
            }
        }

        for (int indexOfColumn = 0; indexOfColumn < SIZE; indexOfColumn++) {
            if (isOnColumn(indexOfColumn)) {
                return GameStatus.getStatusBy(grid[0][indexOfColumn]);
            }
        }

        if (isOnDiagonal() || isOnSubDiagonal()) {
            return GameStatus.getStatusBy(grid[1][1]);
    ...
    public String drawGridIntoString() {
